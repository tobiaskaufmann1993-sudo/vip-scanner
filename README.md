...
شهر ابتدا از GeoIP تأییدشده گرفته می‌شود. اگر DB-IP مختصات معتبر داشته باشد اما نام شهر نداشته باشد، نزدیک‌ترین شهر مهم GeoNames فقط تا شعاع `25 km` و با همان کنترل کشور و دیتاسنتر Cloudflare بازیابی می‌شود. این مرحله صرفاً جاهای خالی DB-IP را تکمیل می‌کند و هیچ شهر موجود DB-IP را جایگزین نمی‌کند. اگر هنوز شهر مطمئنی وجود نداشته باشد، الگوی صریح `Country, City` در نام اصلی فقط در صورت تطابق کشور و توافق نام‌های تکراری استفاده می‌شود؛ در اختلاف‌ها شهر نمایش داده نمی‌شود. شماره بعد از `#` شناسه پایدار همان Fingerprint است، شماره ترتیب لیست نیست. شناسه‌ها ابتدا از بازه چهاره‌رقمی `1000–9999` انتخاب می‌شوند و فقط پس از پرشدن کامل این بازه به بازه پنج‌رقمی می‌روند؛ هیچ شناسه‌ای بیشتر از پنج رقم نیست.
...
نام `Unknown` هیچ‌وقت وارد ساب نهایی نمی‌شود. اگر Cloudflare Trace نتواند IP خروجی را بدهد، ربات IP واقعی همان تونل را از fallback مستقل IP-only می‌گیرد و با DB-IP محلی کشور را تعیین می‌کند؛ سپس سابقه و نام‌های اصلی ساب را بررسی می‌کند. موردی که پس از تمام این مراحل هنوز کشور ندارد منتشر نمی‌شود و گزینه بعدیِ دارای کشور جای آن را می‌گیرد.
...
20. IP خروجی واقعی، کشور و دیتاسنتر مشاهده‌شده Cloudflare را از داخل همان تونل می‌گیرد؛ اگر Trace نتواند IP بدهد، فقط IP عمومی همان تونل را از ipify و در مرحله دوم AWS Check IP می‌گیرد و مکان‌یابی را همچنان با دیتابیس محلی انجام می‌دهد.
...
22. اگر DB-IP نام شهر نداشته باشد ولی مختصات معتبر داشته باشد، فقط نزدیک‌ترین شهر مهم GeoNames در همان کشور و حداکثر فاصله `25 km` بررسی می‌شود؛ کنترل توافق کشور و فاصله دیتاسنتر Cloudflare همچنان اجباری است و شهر DB-IP موجود هرگز با این روش عوض نمی‌شود.
23. اگر پس از آن شهر GeoIP قابل تأیید نباشد، نام اصلی ساب را فقط با الگوی صریح `Country, City` بررسی می‌کند؛ کشور باید با خروجی سازگار باشد و تمام نام‌های همان کانفیگ روی یک شهر توافق داشته باشند، وگرنه شهر حذف می‌شود.
24. کشور از خروجی زنده، DB-IP، سابقه معتبر و نام‌های اصلی سازگار بازیابی می‌شود؛ کانفیگ باقی‌مانده بدون کشور از انتشار حذف می‌شود و هرگز با نام `Unknown` وارد اپ نمی‌شود.
25. DB-IP City و GeoNames Cities ماهی یک‌بار دانلود و در GitHub Actions Cache نگهداری می‌شوند؛ هیچ API سهمیه‌دار GeoIP یا درخواست مکان‌یابی جداگانه برای هر کانفیگ استفاده نمی‌شود.
26. شناسه ۴ یا حداکثر ۵ رقمی از Fingerprint ساخته، برخورد احتمالی کنترل و رجیستری کامل آن در `state.json` حفظ می‌شود؛ شناسه حذف‌شده به سرور دیگر واگذار نمی‌شود.
...
```json
{
  ...
  "naming": {
    ...
    "country_detected": 48,
    "country_unknown": 2,
    "country_unknown_excluded": 2,
    "city_displayed": 39,
    "city_confident": 34,
    "city_from_dbip": 31,
    "city_from_geonames_nearest": 3,
    "city_from_source_name": 5,
    "city_omitted": 19,
    ...
    "geonames_database_available": true,
    "geonames_nearest_city_km": 25.0
  },
  "safety_mode": "normal"
}
```
...
GEONAMES_CITIES_PATH: ".cache/geoip/cities15000.txt"
GEONAMES_NEAREST_CITY_KM: "25"
```

- Cloudflare Trace، IP و کشور خروجی واقعی و کد دیتاسنتر مشاهده‌شده را از داخل Xray می‌دهد.
- اگر Cloudflare Trace فاقد IP باشد، ipify و AWS Check IP فقط برای مشاهده IP عمومی داخل همان Xray استفاده می‌شوند؛ این سرویس‌ها نام کشور یا شهر را تعیین نمی‌کنند.
- DB-IP City به‌صورت محلی همان IP را بررسی می‌کند؛ بنابراین محدودیت API ندارد.
- اگر DB-IP فقط مختصات بدهد و نام شهر نداشته باشد، نزدیک‌ترین شهر مهم در فایل محلی GeoNames با شعاع محافظه‌کارانه ۲۵ کیلومتر بررسی می‌شود. این fallback شهر موجود DB-IP را تغییر نمی‌دهد.
...
- وقتی GeoIP شهر قابل‌تأییدی ندارد، نام اولیه کانفیگ فقط در قالب روشن `Country, City | tag` بررسی می‌شود. کشور نوشته‌شده باید با کشور خروجی یکسان باشد و اگر یک Fingerprint در چند ساب نام‌های شهری متفاوت داشته باشد، هیچ‌کدام نمایش داده نمی‌شود.
...
دادهٔ GeoIP شهر از [DB-IP Lite](https://db-ip.com/db/lite.php) و فهرست شهرهای مهم از [GeoNames](https://download.geonames.org/export/dump/) استفاده می‌شود؛ هر دو تحت مجوز Creative Commons Attribution 4.0 هستند. `index.html` خروجی نیز Attribution هر دو منبع را نمایش می‌دهد.
...
موقعیت IP ذاتاً تقریبی است. شهر GeoIP فقط با تأیید محافظه‌کارانه نمایش داده می‌شود و تضمین نشانی فیزیکی دیتاسنتر نیست. بازیابی GeoNames فقط برای رکورد فاقد نام شهر، در همان کشور و شعاع ۲۵ کیلومتر انجام می‌شود؛ اگر شرایط لازم برقرار نباشد شهر حدس زده نمی‌شود. پس از آن fallback نام ساب فقط با تطابق کشور و توافق نام‌های تکراری مجاز است؛ هر نوع تناقض شهری باعث نمایش فقط نام کشور می‌شود. اگر خود کشور نیز پس از تمام fallbackها ناشناخته بماند، آن کانفیگ اصلاً منتشر نمی‌شود.
